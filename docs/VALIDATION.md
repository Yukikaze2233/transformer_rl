...
按用户明确的视觉修复范围，未采用15刚体动力学重建模板；训练继续使用统一研究动力学。新增标准last-token Transformer对照，配合逐环境/回合去均值统计，形成7种配置的结构比较。完整本机检查为 **1007 passed、10 subtests passed、0 failed、0 skipped，46.91秒**，含CUDA与ONNX一致性检查。

`.gitignore`已验证覆盖模型权重、导出sidecar、运行日志、仿真缓存、录像和本地环境；必要的源码、配置及小型评估证据保留版本管理。

## 修正版几何与稳态评价准备

本阶段曾以CPU-only完成913 passed、24 CUDA skips、10 subtests，包含后来未采用的动力学参数模板。用户明确仅视觉修复后，相关模板与测试已撤回；当前功能是稳态统计、MLP离线基线及视觉审查，最终验证数量以本次后续检查为准。

15视觉组件随动已验证；本轮没有创建或替换动力学模型，所有网络继续采用同一研究物理基线。此前预跑已保存停止，正式对照按用户最新澄清另建队列并记录实际开始时间。
